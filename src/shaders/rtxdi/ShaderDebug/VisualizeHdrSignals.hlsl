...
float4 main(float4 i_position : SV_Position) : SV_Target
{
    ...
    if (g_Const.visualizationMode == VISUALIZATION_OVERLAY_MODE_RESOLVED_COLOR)
        resolutionScale = 1.0;
    ...
    case VISUALIZATION_OVERLAY_MODE_RESERVOIR_M: {
        RTXDI_DIReservoir reservoir = RTXDI_LoadDIReservoir(g_Const.restirDIReservoirBufferParams, reservoirPos, g_Const.inputBufferIndex);
        input = reservoir.M;
        break;
    }

    case VISUALIZATION_OVERLAY_MODE_DIFFUSE_GRADIENT: {
        float4 gradient = t_Gradients[reservoirPos / RTXDI_GRAD_FACTOR];
        input = gradient.x;
        break;
    }

    case VISUALIZATION_OVERLAY_MODE_SPECULAR_GRADIENT: {
        float4 gradient = t_Gradients[reservoirPos / RTXDI_GRAD_FACTOR];
        input = gradient.y;
        break;
    }
                                   
    case VISUALIZATION_OVERLAY_MODE_GI_WEIGHT: {
    ...
    case VISUALIZATION_OVERLAY_MODE_GI_M: {
        RTXDI_GIReservoir reservoir = RTXDI_LoadGIReservoir(g_Const.restirGIReservoirBufferParams, reservoirPos, g_Const.inputBufferIndex);
        input = reservoir.M;
        break;
    }
