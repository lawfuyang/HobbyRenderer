...
float3 GetTransmittanceToTopAtmosphereBoundary(
    uint transmittance_texture_index, float r, float mu) {
...
float3 GetTransmittance(
    uint transmittance_texture_index,
...
float3 GetSunAndSkyIrradiance(
  uint transmittance_texture_index,
  uint irradiance_texture_index, float3 p, float3 normal, float3 sun_direction,
  out float3 sky_irradiance) {
  float r = length(p);
  float mu_s = dot(p, sun_direction) / r;

  // Indirect irradiance (approximated if the surface is not horizontal).
  sky_irradiance = GetIrradiance(irradiance_texture_index, r, mu_s) *
    (1.0 + dot(normal, p) / r) * 0.5;

  // Direct irradiance.
  return ATMOSPHERE.solar_irradiance *
    GetTransmittanceToSun(transmittance_texture_index, r, mu_s) *
    max(dot(normal, sun_direction), 0.0);
}

float3 GetSkyRadiance(
    uint transmittance_texture_index, uint scattering_texture_index,
...
float3 GetSkyRadianceToPoint(
    uint transmittance_texture_index, uint scattering_texture_index,
    float3 camera, float3 p, float shadow_length,
    float3 sun_direction, out float3 transmittance) {
